...
export const Dashboard: React.FC = () => {
  ...
  const handleFetchCommits = async () => {
    ...
    try {
      ...
      if (status === 404) {
        setFetchCommitsError(`"${slug}" not found — if it's a private repo, your token needs the "repo" scope.`);
    ...
    <div className="min-h-screen bg-[#080b12] text-white" style={{ fontFamily: "'IBM Plex Mono', 'Fira Code', monospace" }}>
      <header className="border-b border-white/6 px-6 py-3.5 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-xs font-bold">DD</div>
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs px-2 py-0.5 rounded-full bg-slate-700/70 text-white/80 ring-1 ring-white/10 flex items-center gap-1.5">
            ...
            {githubConnected === true ? <span className="text-green-400 flex items-center gap-1"><CheckCircle size={10} /> GitHub: {githubUser}</span> : githubConfigured ? 'GitHub: Env' : 'GitHub: Mock'}
          </span>
          {view !== 'home' && (
          ...
          <div className="space-y-10">
            <div>
              ...
              <p className="text-white/40 text-sm leading-relaxed max-w-xl">
                Select a commit and run the analysis pipeline. DocAnchor parses changed symbols, scores drift, and surfaces reviewable PRs.
              </p>
            </div>

            {/* GitHub token for real repo analysis */}
              ...
              {isGithubMode ? (
                <>
                  <div className="flex items-center justify-between mb-3">
                    <p className="text-xs uppercase tracking-widest text-white/30">Recent commits · <span className="text-blue-400">{repoInput}</span></p>
                    <button onClick={() => { setCommitMode('mock'); setGithubCommits([]); }} className="text-xs text-white/30 hover:text-white/60 transition-colors">← Use demo scenarios</button>
                  </div>
                  <div className="grid gap-3">{githubCommits.map((c, i) => (
                    <button key={c.full_sha} onClick={() => setSelectedGithubCommit(i)} className={`text-left rounded-xl border p-4 transition-all ${selectedGithubCommit === i ? 'border-blue-500/50 bg-blue-500/8 ring-1 ring-blue-500/20' : 'border-white/8 bg-white/2 hover:border-white/15 hover:bg-white/4'}`}>
                      <div className="flex items-start gap-3">
                        <GitCommit size={14} className={`mt-0.5 shrink-0 ${selectedGithubCommit === i ? 'text-blue-400' : 'text-white/30'}`} />
                        <div className="min-w-0">
                          <div className="flex items-center gap-2 mb-1"><span className="font-mono text-xs text-white/30">{c.sha}</span></div>
                ...
                <>
                  ...
                  <div className="grid gap-3">
                    {loadError ? <p className="text-red-400/90 text-sm">Failed to load scenarios: {loadError}</p> : commits.length === 0 ? <p className="text-white/30 text-sm animate-pulse">Loading scenarios…</p> : commits.map((c, i) => (
                      <button key={c.sha} onClick={() => setSelectedCommit(i)} className={`text-left rounded-xl border p-4 transition-all ${selectedCommit === i ? 'border-blue-500/50 bg-blue-500/8 ring-1 ring-blue-500/20' : 'border-white/8 bg-white/2 hover:border-white/15 hover:bg-white/4'}`}>
                        <div className="flex items-start gap-3">
                          <GitCommit size={14} className={`mt-0.5 shrink-0 ${selectedCommit === i ? 'text-blue-400' : 'text-white/30'}`} />
                          <div className="min-w-0">
                            <div className="flex items-center gap-2 mb-1"><span className="font-mono text-xs text-white/30">{c.sha}</span><span className="text-xs text-white/25 px-1.5 py-0.5 rounded bg-white/5">{c.branch}</span></div>
                            <p className="text-sm text-white/80 leading-snug">{c.message}</p>
            ...
            <button onClick={handleRun} disabled={loading || (!isGithubMode && commits.length === 0)} className="flex items-center gap-2.5 px-6 py-3 rounded-xl bg-blue-600 hover:bg-blue-500 disabled:opacity-40 disabled:cursor-not-allowed text-white font-semibold text-sm transition-colors">
        ...
        {view === 'dashboard' && metrics && health && (
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="col-span-2 md:col-span-1 rounded-2xl border border-white/8 bg-white/3 p-5 flex flex-col items-center justify-center"><FreshnessGauge score={health.freshness_score} /><p className="text-xs text-white/30 mt-2">Repo Freshness</p></div>
              {[
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex gap-1 ml-auto">
                {[0, 15, 40, 70].map(min => (
                  <button key={min} onClick={() => setFilterMin(min)} className={`px-3 py-1 rounded-lg text-xs transition-colors ${filterMin === min ? 'bg-white/10 text-white' : 'text-white/30 hover:text-white/60'}`}>{min === 0 ? 'All' : `≥${min}`}</button>
                ))}
              </div>
            ...
            <div className="space-y-2">
              {sortedResults.length === 0 ? <div className="text-center py-12 text-white/25 text-sm">No blocks above this threshold — docs look fresh!</div> : sortedResults.map((result, i) => (<DriftCard key={result.doc_block_id} result={result} rank={i + 1} repo={health.repo} />))}
            </div>

            <p className="text-center text-xs text-white/20 pb-4">Last analyzed {new Date(health.last_analyzed).toLocaleTimeString()} · {health.repo}</p>
          </div>
        )}
      </main>
    </div>
